...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            ResultSet rs = stm.executeQuery("select * from admin where branchName='"+username+"' and password='"+password+"'");
            
            if(rs.next()){
                request.getSession().setAttribute("username",rs.getString("branchName"));
                request.getSession().setAttribute("branchid", rs.getInt("branchID"));   
